...
public sealed record UnfreezeBalanceCommand(Guid MerchantId, string Currency);

public sealed class UnfreezeBalanceHandler(IUnitOfWork uow)
{
    public async Task<Result<BalanceSummaryDto>> HandleAsync(
        UnfreezeBalanceCommand cmd, CancellationToken ct = default)
    {
