...
public sealed record ReactivateEmployeeCommand(Guid MerchantId, Guid EmployeeId, string ActorRole);
 
public sealed class ReactivateEmployeeHandler(IUnitOfWork uow)
{
    public async Task<Result<EmployeeDto>> HandleAsync(
        ReactivateEmployeeCommand cmd, CancellationToken ct = default)
    {
