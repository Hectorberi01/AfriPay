...
public sealed record FreezeBalanceCommand(Guid MerchantId, string Currency);
 
public sealed class FreezeBalanceHandler(IUnitOfWork uow)
{
    public async Task<Result<BalanceSummaryDto>> HandleAsync(
        FreezeBalanceCommand cmd, CancellationToken ct = default)
    {
